...
class Preprocess():
    ...
    def munge_data(self, verbose=True):
        """
        ...
        for i in range(1, row):
            ...
            if int(df_arr[i, 2]) == 1:
                ...
                seq[:-1, 0] = (seq[:-1, 0] - MIN_X) / (MAX_X - MIN_X)
                seq[:-1, 1] = (seq[:-1, 1] - MIN_Y) / (MAX_Y - MIN_Y)
        ...
        try:
            self.data_list = pad_sequences(self.data_list, value=9, dtype=np.float64, padding='post')
