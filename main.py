...
MAKE SURE to unpack your data and set the "file_dir" variable to
...
data_loaded = DataLoad(file_dir, csv_file, center)

# Munge the data. Arguments see the class
data_loaded.munge_data(11, sl, db)
# Center the data
data_loaded.center_data(center)
...
if True:
    ...
    while i < max_iterations and not early_stop:
        ...
        if i % plot_every == 0:
            ...
            print("The best AUC is %6s and the best index value is %s" % (auc_best, best_auc_idx))
            step += 1
...
#                 plt.close('all')
#         np.savetxt(dir + 'traj_shiny.csv', seq_exp, delimiter=',')
#         export_lab = np.stack(export_lab, axis=1).T  #
#         np.savetxt(dir + 'traj_shiny_log.csv', export_lab, delimiter=',')
