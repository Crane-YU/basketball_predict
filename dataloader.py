...
class DataLoad():
    ...
    def munge_data(self, height=11.0, seq_len=10.0, dist=3.0, verbose=True):
        ...
        if height < 9.0:
            print('Please note that height is measured from ground.')
        ...
        # Check is the data frame has the columns we want
        if not 'x' and 'y' and 'z' and 'game_clock' and 'rankc' in df.columns:
            print('We miss columns in the dataframe')
        ...
        for i in range(1, row):
            ...
            if int(df_arr[i, 5]) == 1:  # rankc is 1
                ...
                seq = df_arr[start_ind:end_ind, :4]  # get the cols for x, y, z, gameclock
                dist_xyz = np.linalg.norm(seq[:, :3] - self.center, axis=1)  # get the distance to the basket
                ind_crit = np.logical_and((seq[:, 2] > height), (dist_xyz > dist))  # Indices satisfying the criteria
                if np.sum(ind_crit) == 0:
                    continue  # No sequence satisfies the criteria for this i
                length_i, i = return_large_true(ind_crit)
                seq = seq[i:i+length_i, :]  # Excludes rows where z coordinate is lower than "height"
                try:
                    # NOTE: this implementation assumes that the time is ticking down
                ...
                if seq.shape[0] >= seq_len:
                    self.data3.append(seq[-seq_len:])  # Add all the sequences to the list
                    # print("Enter the 2nd if-statement, shape is:", len(self.data3))
        ...
        try:
            ...
            print("label shape after is:", self.labels.shape)
